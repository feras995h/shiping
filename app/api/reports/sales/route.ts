...
export async function GET(request: NextRequest) {
  try {
    ...
    return ApiResponseHandler.serverError('فشل في جلب تقارير المبيعات')
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    const validatedData = salesReportSchema.parse(body)
    ...
    return ApiResponseHandler.success(report, 'تم إنشاء تقرير المبيعات بنجاح')
  } catch (error) {
